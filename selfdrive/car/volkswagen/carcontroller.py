...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, visual_alert, audible_alert, leftLaneVisible, rightLaneVisible):
    ...
    #                                                                         #
    # --------------------------------------------------------------------------
    if (frame % P.MOB_STEP == 0) and CS.CP.enableGasInterceptor and not CS.out.cruiseState.stockCCDisabled:
    ...
    #                                                                         #
    # --------------------------------------------------------------------------
    if (frame % P.GAS_STEP == 0) and CS.CP.enableGasInterceptor and not CS.out.cruiseState.stockCCDisabled:
      apply_gas = 0
      if enabled:
